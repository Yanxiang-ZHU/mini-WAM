"""Train the World Model (flow-matching visual subgoal generator)."""
...
def train_one_epoch(model, loader, opt, scaler, device, amp):
    ...
    for batch in loader:
        ...
        subgoal = batch["subgoal"].unsqueeze(1).to(device)  # (B,1,48,48)
        ...
        with torch.amp.autocast("cuda", dtype=torch.float16, enabled=amp):
            loss = model.loss(subgoal, history, lang_ids, subtask_ids, meta)
...
def main():
    ...
    ap.add_argument("--config", default="configs/world_model.yaml")
